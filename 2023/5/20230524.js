...
function draw() {
  ...
  ox += map(noise(frameCount*0.01*speedX), 0, 1, -4, 4);
  oy += map(noise(frameCount*0.01*speedY), 0, 1, -4, 4);
  ...
  for (let i = 0; i < pointNumber; i++) {
    ...
    fill(d*cos(frameCount-d2+d2), d*cos(frameCount+d2+d), d*tan(i/d), 1);
    stroke(d*cos(frameCount-d2+d2), d*cos(frameCount+d2+d), d*tan(i/d), 5);
    if (cell) {
      if(d > d2) {
    fill(d2*cos(frameCount+d2+d), 1);
      }
      beginShape();
