...
function preload() {
  girlImg = loadImage("20230201.png")
}
...
function setup() {
  ...
  let newCanvasX = (windowWidth - size)/2;
  let newCanvasY = (windowHeight - size)/2;
...
function girl(ox, oy, f) {
  for (let i = 0; i < 36; i++){
    ...
    let angle = map(noise(xoff, yoff, frameCount*0.5), 0, 1, 0, 360)
    push();
    translate(ox, oy)
    x = i * tan(angle * frameCount*0.5)
    y = i * cos(i + angle)
    if(f == 1) scale(-1, 1)    
