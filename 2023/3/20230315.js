...
function setup() {
	...
	for(let x = 0; x < width+grid; x+=grid) {
		for(let y = 0; y < height+grid; y+=grid) {
			...
			let r = map(noise(x*0.001, y*0.001, tan(ox)/sin(oy)), 0, 1, 0, 30)
...
function draw() {
	for(let i = 0; i < points.length; i++) {
		push();
		let angle = map(noise(points[i].x*0.01, points[i].y*0.01, frameCount*0.001), 0, 1, 0, 360)
	...
	counter = sin(frameCount*0.05)*grid*0.3
}