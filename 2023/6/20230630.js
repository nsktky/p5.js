class Lightning {
  constructor() {
    ...
    this.noiseScale = 0.01; // パーリンノイズのスケール
    this.reset();
    colorMode(HSB, 360, 50, 100, 200)
  }

  reset() {
    this.path = [];
    let start = createVector(width/2, 0);
    this.path.push(start);
    this.noiseOffset = random(100);
    this.currentStep = 0;
    this.opacity = 255;
    this.speed = random(5, 10); // 追加: 雷のスピード
    this.isAlive = !this.isAlive
  }

  generateStep() {
    let lastPoint = this.path[this.path.length - 1];
    let newX = lastPoint.x + map(noise(this.noiseOffset), 0, 1, -15, 15);
  ...
  draw() {
    stroke(map(this.currentStep, 0, this.maxSteps, 30, 120), this.noiseOffset, this.opacity, this.opacity);
    // noFill();
    fill(360, 40)
    beginShape();
    for (let point of this.path) {
      ...
      vertex(point.x, point.y);
      // vertex(point.x, point.y);
      vertex(point.x-random(-5, 5), point.y-100);
    }
    endShape();
  }
}
