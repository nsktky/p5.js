let points = [];
let grid;
let seed;

function setup() {
  createCanvas(windowWidth, windowHeight);
  background(25);
  angleMode(DEGREES)

  colorMode(HSB, 360, 100, 100, 100)

  grid = width / 100;
  for (let x = 0; x < width + grid; x += grid) {
    for (let y = 0; y < height + grid; y += grid) {
      let p = createVector(x, y);
      points.push(p);
    }
  }

  seed = random(100000000000);
}

function draw() {
  // background(250, 5)
  randomSeed(seed)
  for (let i = 0; i < points.length; i++) {
    push();
    let angle = map(noise(points[i].x * 0.001, points[i].y * 0.001),0,1,0,300)
    let h = map(noise(points[i].x * 0.01, points[i].y * 0.01, frameCount*0.01),0,1,0,480)
    let s = map(noise(points[i].x * 0.1, points[i].y * 0.1, frameCount*0.01),0,1,0,100)
    let b = map(noise(points[i].x * 0.01, points[i].y * 0.01, frameCount*0.03),0,1,0,100)

    points[i].add(createVector(cos(angle), sin(angle)))
    if(points[i].x < 0) points[i].x = width
    if(points[i].x > width) points[i].x = 0
    if(points[i].y < 0) points[i].y = height
    if(points[i].y > height) points[i].y = 0

    translate(points[i].x, points[i].y)
    stroke(h, s, b, 100)
    let step = random(10)
    line(0, 0, sin(frameCount*step) * grid , cos(frameCount*step) * grid)
    pop()
  }

}